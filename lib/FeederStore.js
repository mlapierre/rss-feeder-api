    ...
    saveArticles(feed).then(function(res) {
      ...
      return db.allDocs({
        ...
        return docs.rows.filter(function(res) {
          return res.doc.feed_id === feed._id && !res.doc.read_at;
        });
      }).catch(function(err) {
        deferred.reject(err);
      });
    }).then(function(res) {
      unread_count = res.length;
      feed.articles = [];
      db.upsert(feed._id, function(doc) {
        // If doc doesn't have an _id property, the feed is new
        if (!doc.hasOwnProperty('_id')) {
          feed.unread_count = res.length;
          return feed;
        }
        ...
        userdb.allDocs({
          ...
          res.rows.forEach(function(row) {
            ...
            userdb.upsert(row.doc._id, function(doc) {
              ...
              userdb.get(tag._id).then(function(res) {
                console.log(doc + '; ' + res);
              });
            });
          });
